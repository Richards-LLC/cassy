...
- `cas-cli/src/factory_{context_reset,isolation,preflight,target_cache}.rs` plus `cas-cli/src/factory_*` guards — factory safety and host/build checks.
...
- `cas-cli/src/builtins/codex/` — the only per-harness files the prefix-neutral catalog cannot share (the Codex no-hooks checklist and two `openai.yaml` policies); every other catalog entry embeds the canonical file, and the task-verifier's `tools:` frontmatter is generated per harness around `agents/task-verifier.body.md`.
...
- `crates/cas-mux/src/{backend,pane}/` plus `crates/cas-mux/src/{mux,pty,render,harness}.rs` — terminal backends and pane operations.
- `crates/cas-pty/{src,conformance}/` — runtime adapters and pinned harness contract receipts.
