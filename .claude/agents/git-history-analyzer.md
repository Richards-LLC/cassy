...
- **The cas-35c5 / cas-XXXX pattern** in commit messages is the Cassy task-ID breadcrumb. Commits ending in `(cas-XXXX)` link back to tasks — fetching the task via `mcp__cas__task action=show id=cas-XXXX` is often the fastest way to understand "why."
