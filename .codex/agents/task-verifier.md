...
model: inherit
tools: Read, Grep, Glob, Bash, mcp__cs__task, mcp__cs__verification, mcp__cs__rule, mcp__cs__search, mcp__cs__coordination
managed_by: cas
---
...
The task record names the delivery: `deliverables.files_changed`, `deliverables.commit_hash`, and the target branch (`Target: … @ <branch>`). In factory mode, work in the worker's clone (`mcp__cs__coordination action=worker_status` gives its path). Diff against the task's own delivery base, never a fixed commit count:
