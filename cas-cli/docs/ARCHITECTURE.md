...
| `mcp/` | MCP server: `server/` (CasCore with cached OnceLock stores), `tools/` — 13 action-dispatched meta-tools (`memory`, `task`, `rule`, `skill`, `coordination`, `search`, `system`, `verification`, `knowledge`, `team`, `pattern`, `spec`, `artifact`; plus `mcp_search`/`mcp_execute` when a proxy is configured), each fanning out to handlers in `core/` and `service/` — `daemon.rs` (embedded background maintenance), `socket.rs` (notification socket) |
