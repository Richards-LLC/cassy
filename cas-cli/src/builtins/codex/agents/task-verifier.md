...
```
mcp__cs__verification action=add task_id=<id> status=error summary="BUG: close path blocked. Signal: [signal]. Error: [message]" confidence=0.0
```

## Demo-statement evidence mode

Fetch the task with `mcp__cs__task action=show` and inspect fields and notes
...
`mcp__cs__task action=dep_list id=<epic-id>` and fetch every child, including
...
1. Locate `~/.cas/artifacts/<task-id>/LEDGER.md`. The task notes and eventual
   ...
   Record that rejection with `mcp__cs__verification action=add` and stop.
...
```
mcp__cs__task action=show id=<task-id>
mcp__cs__factory action=worker_status
...
```
mcp__cs__verification action=add task_id=<id> status=approved summary="Work complete and production-ready. Implementation follows codebase patterns with clean error handling and appropriate abstractions." confidence=0.95 files_reviewed="file1,file2"
```
