...
`mcp__cs__memory` tool for entry lifecycle operations, and use
`mcp__cs__search` when you need to find a memory by topic. Store useful
project facts proactively, especially after a non-trivial diagnosis or design
decision.

## Valid Actions

The list below is the dispatch order for `mcp__cs__memory`; keep it aligned
with the live service.

**Valid `mcp__cs__memory` actions** (exact list — do not invent others): `remember`, `get`, `list`, `update`, `delete`, `archive`, `unarchive`, `helpful`, `harmful`, `mark_reviewed`, `recent`, `set_tier`, `opinion_reinforce`, `opinion_weaken`, `opinion_contradict`.

## Common Operations

- **Remember**: `mcp__cs__memory action=remember title="..." content="..." entry_type=learning`
- **Find**: `mcp__cs__search action=search query="..." doc_type=entry`
- **Read**: `mcp__cs__memory action=get id=<entry-id>`
- **List**: `mcp__cs__memory action=list scope=project limit=20`
- **Revise**: `mcp__cs__memory action=update id=<entry-id> content="..."`
...
- **Recent**: `mcp__cs__memory action=recent limit=10`
