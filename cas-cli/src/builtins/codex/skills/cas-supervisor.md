...
- **Never reply to the `From:` label:** for a verified Commander row, use the exact reply command printed beside it (`mcp__cs__coordination action=message target=operator in_reply_to=N message=…`); the label is display text, not a routing target.
