...
`mcp__cs__factory action=gc_cleanup force=true older_than_secs=86400` terminally
...
- **Wedged:** `cas factory kill <worker>`, then respawn. It SIGKILLs the worker's process group and resets its leased tasks (same semantics as `mcp__cs__task action=reset`) only once death is confirmed; `cas factory kill --help` documents process resolution and the PID-recycling guard. Investigate before passing `--force`.
- **Starved:** do not kill. Come back in 2 minutes; if it re-classifies as `wedged`, proceed to the kill path.
- **Dead:** no kill needed. The `kill` verb is still safe to run (`skipping SIGKILL` + task reset runs); or manually `mcp__cs__task action=reset id=<task-id>`.
...
- Check liveness with `mcp__cs__factory action=worker_status` before declaring a worker idle or dead.
...
Run `mcp__cs__factory action=worker_status`, then the `is-wedged` / `debug` / `kill` triad above. Salvage committed work with `mcp__cs__factory action=worktree_merge id=<worker> task_id=<task-id>` before any cleanup, and message the replacement worker what already landed.
...
**Diagnosis:**
1. Confirm a fresh heartbeat with `mcp__cs__factory action=worker_status`
...
3. Check prompt delivery state with `mcp__cs__coordination action=message_status notification_id=<id>` (the id the `message` call returned). It reports transport handoff, wake observations and confirmation separately; the queue columns behind it are `processed_at, acked_at`. A set `processed_at` records transport processing; `acked_at` records queue acknowledgement. Neither is assignment acceptance or execution proof. Use `queue_ack` for durable supervisor notifications and `message_ack` for prompt-message receipts; lifecycle relay acknowledgements reconcile linked rows. Neither replaces `task action=start`. Missing prose ACK alone is not a recovery trigger.
...
shutdown requests. Run `mcp__cs__factory action=restart_spawn_queue`. It
...
`mcp__cs__factory action=worker_status` prints a `context:` line per worker, banded by the share of the model's context window in use:
...
1. Send: `mcp__cs__coordination action=message target=<worker> summary="Context near limit — commit now" message="Your context is near the limit. Commit any in-progress work immediately (git add / git commit), then report what you committed."`
2. Wait for the commit confirmation (watch `mcp__cs__factory action=worker_activity`).
...
4. Once work is committed and the worker is idle: `mcp__cs__factory action=recycle_worker target=<worker>`. It restarts the same name with its recorded recipe and keeps the worktree.
...
2. Shut down the affected worker immediately: `mcp__cs__factory action=shutdown_workers worker_names=<worker>`. Do not attempt to salvage the session.
...
5. Free the task with `mcp__cs__task action=reset id=<task-id>`, then spawn a fresh worker on it (`task_id=<task-id>`); point it at the good commits in the assignment.
