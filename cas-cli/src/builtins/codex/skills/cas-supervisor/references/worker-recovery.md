...
`mcp__cs__coordination action=gc_cleanup force=true older_than_secs=86400` terminally
...
Run `mcp__cs__coordination action=worker_status`, then the `is-wedged` / `debug` / `kill` triad above. Salvage committed work with `mcp__cs__coordination action=worktree_merge id=<worker> task_id=<task-id>` before any cleanup, and message the replacement worker what already landed.
...
shutdown requests. Run `mcp__cs__coordination action=restart_spawn_queue`. It
...
`mcp__cs__coordination action=worker_status` prints a `context:` line per worker, banded by the share of the model's context window in use:
...
4. Once work is committed and the worker is idle: `mcp__cs__coordination action=recycle_worker target=<worker>`. It restarts the same name with its recorded recipe and keeps the worktree.
...
2. Shut down the affected worker immediately: `mcp__cs__coordination action=shutdown_workers worker_names=<worker>`. Do not attempt to salvage the session.
