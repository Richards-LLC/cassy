...
2. Choose exactly one task. Run `mcp__cs__task action=show id=<task-id>`,
   then `mcp__cs__task action=start id=<task-id>` before editing. A
...
7. Close with `mcp__cs__task action=close id=<task-id> reason="..."`, then
...
`ToolSearch(query="select:mcp__cs__task")` once, then call it; lookup
