...
- Use `mcp__cs__coordination action=message`, not the built-in `SendMessage`, from your first ready-ping at spawn onward. `SendMessage` isn't blocked — in factory mode a PreToolUse hook auto-routes it onto the same CAS queue and returns success (cas-f32b) — but it only carries what that layer can parse. Call the coordination tool directly.
