...
**`mcp__cs__coordination` actions workers routinely use**: `message`, `message_ack`, `message_status`, `inbox_poll`, `whoami`, `heartbeat`, `remind`. Load only `mcp__cs__task` and `mcp__cs__coordination`; the supervisor `mcp__cs__factory` tool (fleet, worktree, server, database, loop and queue control) is not yours, apart from `server_start`/`server_list` when a task needs a long-lived server (see cas-servers).
