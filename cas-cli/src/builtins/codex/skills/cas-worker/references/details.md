...
**Need a database?** You cannot create a Neon branch or hold a Neon credential. Ask the supervisor: `mcp__cs__coordination action=message target=supervisor blocker=true message="db branch for <task-id>: <why>"`. Its `db_branch_create` writes `DATABASE_URL` to `.env.cas-db` in your worktree. Source that file; never commit it or print it. The branch is deleted when your task closes.
