...
```
mcp__cs__coordination action=server_start command="npm run dev" cwd=<dir> port=3000
mcp__cs__coordination action=server_list
mcp__cs__coordination action=server_stop id=<id>
```
...
```
mcp__cs__coordination action=remind target=<your-name> remind_delay_secs=600 \
...
`mcp__cs__coordination action=inbox_poll` **first**, before anything else, to pull the
