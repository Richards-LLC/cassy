...
Invoke `mcp__cs__memory` with `action=remember` to create or update one pointer
...
Find an existing pointer first: `mcp__cs__search action=search query="<title>" doc_type=entry`.
