...
For frameworks the built-in plugins do not cover (`fallow list --plugins` shows the active ones).
...
## Agent hooks: do not install them from this skill

Fallow can install its own agent gate (`fallow setup-hooks`, and its successor
`fallow agent install`). Both write a skill, hooks and an `AGENTS.md` block into
`.claude/` and the project root, where they collide with Cassy-managed files.
Do not run either. `fallow init --hooks` (a Git pre-commit hook under
`.git/hooks/`) is a separate, human-facing path; add it only when the user asks.
