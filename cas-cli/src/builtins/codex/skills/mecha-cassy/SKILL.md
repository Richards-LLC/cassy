...
Set this machine up once with `cas integrate mecha-cassy` (see [references/registration.md](references/registration.md)), then dispatch from a Cassy-connected harness with `mcp__cs__mcp_execute`. A bounded one-shot process with no live proxy uses the proxy-less route in that same reference instead.
