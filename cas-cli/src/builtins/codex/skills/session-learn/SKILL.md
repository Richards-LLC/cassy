...
This skill is adapted from `third-brain-v5-skills/skills/session-learn` (MIT). The third-brain version writes to a wiki tree. This version writes to Cassy memory through `mcp__cs__memory action=remember`, so findings go through Cassy's dedup, embedding and recall.
...
2. **Dedupe.** For each candidate, run `mcp__cs__search action=search query="<the finding>"`. If a near-duplicate exists, do not draft it again; note the existing memory's ID in `dedup_hits`.
...
5. **Store.** For each accepted draft with empty `dedup_hits` and `confidence ≥ 0.6` (≥ 0.5 for corrections), run `mcp__cs__memory action=remember content="<content>" entry_type=<entry_type> scope=<scope> tags="<tags>"`. The overlap gate there is the backstop.
