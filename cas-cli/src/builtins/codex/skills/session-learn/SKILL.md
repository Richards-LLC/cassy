...
Borrowed from `third-brain-v5-skills/skills/session-learn` (MIT, reference_third_brain_v5_skills_borrow_source) and adapted to the Cassy memory schema. The third-brain version writes to a wiki tree; this version writes to the Cassy memory + rule stores via `mcp__cs__memory remember` so findings benefit from Cassy's existing dedup, embedding, and recall pipeline.
...
- `third-brain-v5-skills/skills/session-learn/SKILL.md` — upstream pattern. Diffs: third-brain writes to a wiki tree; this skill writes via `mcp__cs__memory remember` so findings inherit Cassy dedup and recall.
