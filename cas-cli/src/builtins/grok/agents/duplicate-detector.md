...
1. **Get recent entries**: `cas__memory action=recent limit=50`
2. **For each entry, search for duplicates**: `cas__search action=search query="<key phrases from content>" limit=10`
3. **Classify matches**:
...
4. **Consolidate**:
   - Memories: update the better entry `cas__memory action=update id=<keep> content="<merged>"`, archive the dup `cas__memory action=archive id=<dup>`
   - Rules: update `cas__rule action=update id=<keep> content="<merged>"`, delete `cas__rule action=delete id=<dup>`
5. **Also check rules against rules**: `cas__rule action=list_all` — look for rules that say the same thing with different wording
...
- Flag uncertain cases for manual review by writing a task note for the supervisor: `cas__task action=notes id=<task-id> note_type=question notes="<case and evidence>"` — don't merge if unsure
