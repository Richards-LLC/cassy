...
- **Repository-aware:** CAS development happens on `Richards-LLC/cassy`. Use its `origin` remote for CAS issues and PRs; treat any separately configured `upstream` remote as a different repository and follow its policy.
- **Task creation:** For each high-priority theme (confidence=high, count ≥ 5), offer to create a CAS epic via `cas__task action=create task_type=epic title="Address <theme>" priority=1`. Do not create automatically — the caller decides.
