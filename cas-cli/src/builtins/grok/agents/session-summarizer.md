...
1. **Get session context**: `cas__search action=context`
2. **List tasks touched**: `cas__task action=mine`
3. **Get recent memories**: `cas__memory action=recent limit=20`
4. **Check for open/in-progress tasks**: `cas__task action=list status=in_progress`
...
6. **Store summary**:
   ```
   cas__memory action=remember content="<summary>" title="Session Summary - [Date]" entry_type=context tags="session,summary"
   ```
