...
```
cas__task action=list status=open limit=100
cas__search action=search query="<the issue's subject in your own words>"
```
...
```
cas__task action=list task_type=epic status=open
```
...
```
cas__task action=create task_type=epic title="<intake> burn-down v<N>: <theme> (GH #<lo>–#<hi>)" priority=1
```

Then, for each new issue, one task:

```
cas__task action=create title="<what will be true when this is done> (GH #<n>)" \
...
```
cas__task action=dep_remove id=<blocked task> to_id=<merged blocker>
```
