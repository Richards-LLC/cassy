...
not a nitpick. Twelve sections, 0–11; section 0 gates the rest.

## The two-minute version (start here)
...
- [ ] The hero is a figure that shows the argument, visible above the fold at 1280×800 and at 390×844.
      A paragraph, a row of numbers, or a table of everything is not a hero.
...
- [ ] Four headless renders exist — 1280 and 390 px, light and dark — and were looked at: no text pair
      ...
      glyph or mark, no page-level horizontal scroll at 390 px.
- [ ] The `--strict` visual-QA receipt (or its stated fallback) from the `cas-ui-craft` critique
      rubric is `PASS`; every allowlisted exception names a selector, finding type, and specific
      reason. Only the small `visual-qa.md` is ever committed beside the report.
- [ ] The tokens in `:root` are the project's `DESIGN.md` or `tokens.css`, by name; the brief
      says which.
...
- [ ] Executive explanatory reports do not invent a decision, ask, or stat strip that their selected type
      does not require.
...
- [ ] Readable at 390 px wide with no horizontal scroll except inside wide tables.
...
- [ ] For an executive brief: the hero, the stat strip, and one chart fit one screen at 1280×800.
