...
Use `cas__search` to find information across Cassy content and code. Choose the right action for the job:

## Which Action to Use

**`search`** — conceptual queries across memories, tasks, rules, skills, and indexed code:
`cas__search action=search query="authentication flow" doc_type=entry`
...
`cas__search action=code_search query="user authentication" kind=function language=rust`

**`grep`** — exact regex matching in indexed files. Use `pattern`, optionally `glob`, `before_context`, `after_context`, and `case_insensitive`:
`cas__search action=grep pattern="TODO:" glob="*.rs"`
...
`cas__search action=search query="deadlock module:cas-mcp severity:critical"`
`cas__search action=search query="track:bug problem_type:runtime_error"`
...
The list below is the dispatch order for `cas__search`; `impact_report` is the alias accepted by the same handler as `skill_impact`.

**Valid `cas__search` actions** (exact list — do not invent others): `search`, `retrieval_feedback`, `retrieval_metrics`, `skill_impact`, `impact_report`, `context`, `context_for_subagent`, `observe`, `entity_list`, `entity_show`, `entity_extract`, `code_search`, `code_show`, `grep`, `blame`, `history`.
