...
- **Commander operator replies:** when an inbound Commander message is stamped `operator … verified` and includes `notification_id=N`, answer it with `cas__coordination action=message target=operator in_reply_to=N summary="..." message="..."`. The hub routes `operator` to the originating paired device and reports `queued for <device>` while offline; do not redirect this response to `supervisor`.
