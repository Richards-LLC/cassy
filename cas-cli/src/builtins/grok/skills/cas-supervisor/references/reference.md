...
| `cli` | string | Explicit CLI backend for this spawn: `claude`, `codex`, `grok`, or `opencode`. OpenCode supports the QwenCloud Token Plan default and explicit Alibaba PAYG lanes, each with a live conformance receipt. If omitted, resolves through factory config, then stock fallback. |
| `model` | string | Explicit model name. Codex uses `gpt-5.6-luna` at `xhigh` for standard/taste work and `gpt-5.6-sol` at `high` for heavy/frontier work; genuinely light work uses Grok Composer/low (no `-codex` suffix; bare `gpt-5.6` is invalid). Luna must not use lower effort; `max`/`ultra` await Cassy vocabulary and Codex-pin validation. Terra is suspended (2026-08-25; operator decision pending). Claude: `opus` for exceptional architecture/safety/rescue/challenge; Sonnet is not a normal worker lane. Grok: `grok-4.5` or `grok-composer-2.5-fast` (from `grok models`). OpenCode defaults to `qwencloud/qwen3.8-max` on Token Plan; `alibaba/qwen3.8-max` and `alibaba-cn/qwen3.8-max` select PAYG explicitly. Passed as `-m`/`--model`. If omitted, resolves through factory config, then the OpenCode Token Plan default. |
