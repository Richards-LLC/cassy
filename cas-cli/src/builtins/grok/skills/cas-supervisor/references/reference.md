...
| `model` | string | Explicit model name. Registry routes are Claude `haiku-4.5`/low for light and `opus-5`/high for taste, Codex `gpt-5.6-luna`/xhigh for standard and `gpt-5.6-sol`/high for heavy. Terra is standing-suspended; never spawn it. Luna must not use lower effort. Grok models are `grok-4.5` and `grok-composer-2.5-fast`, but provider capacity is not an active registry lane. OpenCode defaults explicitly to `qwencloud/qwen3.8-max` on the receipt-gated Token Plan route; `alibaba/qwen3.8-max` and `alibaba-cn/qwen3.8-max` select PAYG explicitly. Passed as `-m`/`--model`. If omitted, resolves through factory config, then the selected harness's stock default. |
