...
| `cli` | string | Explicit CLI backend for this spawn: `claude`, `codex`, `grok`, or `opencode`. OpenCode is implementation-complete-pending-conformance and remains gated by T7's live receipt. If omitted, resolves through factory config, then stock fallback. |
| `model` | string | Explicit model name. Codex uses `gpt-5.6-luna` at `xhigh` for standard/taste work and `gpt-5.6-sol` at `high` for heavy/frontier work; genuinely light work uses Grok Composer/low (no `-codex` suffix; bare `gpt-5.6` is invalid). Luna must not use lower effort; `max`/`ultra` await Cassy vocabulary and Codex-pin validation. Terra is suspended (2026-08-25; operator decision pending). Claude: `opus` for exceptional architecture/safety/rescue/challenge; Sonnet is not a normal worker lane. Grok: `grok-4.5` or `grok-composer-2.5-fast` (from `grok models`). OpenCode: `alibaba/qwen3.8-max` pending T7 conformance. Passed as `-m`/`--model`. If omitted, resolves through factory config, then backend stock fallback. |
| `effort` | string | Explicit reasoning effort. Cassy vocabulary: `minimal` \| `low` \| `medium` \| `high` \| `xhigh` (alias `x-high`). Mapping: Claude `--effort`; Codex `--config model_reasoning_effort=<v>`; Grok `--reasoning-effort`; OpenCode generated primary-agent `variant` (Qwen: `low`, `medium`, `xhigh`). If omitted, resolves through factory config, then stock fallback. For multi-step Claude workers prefer `high` as the ceiling — see [model-selection.md](model-selection.md). |
...
cas__task action=update id=cas-abc1 assignee=<worker-name>
...
cas__task action=transfer id=cas-abc1 to_agent=<worker>
```
...
cas__task action=reset id=cas-abc1

# Step 2: Assign to the new worker
cas__task action=update id=cas-abc1 assignee=<new-worker>

# Step 3: Notify the new worker
cas__coordination action=message target=<new-worker> summary="..." message="..."
```
...
```
# 1. Create
cas__task action=create title="Fix login bug" priority=high \
  description="..." acceptance_criteria="..."

# 2. Assign (this is what causes the worker to pick it up)
cas__task action=update id=cas-abc1 assignee=<worker>
...
```
cas__coordination action=message target=worker-1 \
...
cas__coordination action=message target=<worker> urgent=true \
  summary="..." message="Stop — you're editing the wrong file. Switch to ..."

# Shorthand — forces urgent even without the flag
cas__coordination action=interrupt target=<worker> \
...
```
cas__task action=notes id=cas-abc1 notes="Progress update" note_type=progress
```
