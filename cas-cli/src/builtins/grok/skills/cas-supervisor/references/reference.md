...
| `model` | string | Explicit model name. Registry routes are Claude `haiku-4.5`/low for light and `opus-5`/high for taste, Codex `gpt-5.6-luna`/xhigh for standard and `gpt-5.6-sol`/high for heavy. Terra is standing-suspended; never spawn it. Luna must not use lower effort. Claude Opus also fits exceptional architecture/safety/rescue/challenge; Sonnet is not a normal worker lane. Grok models are `grok-4.5` and `grok-composer-2.5-fast`, but provider capacity is not an active registry lane. Passed as `-m`/`--model`. If omitted, resolves through factory config, then backend stock fallback. |
