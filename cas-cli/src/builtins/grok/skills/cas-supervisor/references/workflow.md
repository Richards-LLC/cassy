...
1. Search before planning — check all three sources for prior art:
   ...
   Grep pattern="<feature-name>" or cas__search action=search query="<keywords>" doc_type=code
   ```
2. Create EPIC: `cas__task action=create task_type=epic title="..." description="..."`
...
```
cas__task action=create title="..." description="..." risk=none
cas__factory action=spawn_workers count=1 isolate=true cli=codex model=gpt-6-sol effort=medium task_id=<task-id>
```
...
1. Spawn workers:
   ```
   cas__factory action=spawn_workers count=N isolate=true cli=codex model=gpt-6-sol effort=medium
   ```
...
cas__factory action=spawn_workers count=1 isolate=true cli=codex model=gpt-6-luna effort=xhigh
...
cas__factory action=spawn_workers count=1 isolate=true cli=claude model=claude-opus-5-5 effort=high
...
3. Confirm they are live: `cas__factory action=worker_status summary_mode=true`
...
`cas__factory action=worktree_merge` is the worker merge path. It resolves
...
```
cas__factory action=worktree_merge id=<worker> task_id=<task-id>
```
...
2. Confirm: `cas__factory action=epic_status id=<focused-epic>` and/or `cas__task action=list status=awaiting_merge`.
   ...
   **User-facing delivery? Independent QA first (cas-619f).** When the park reports
   ...
   `cas__factory action=spawn_workers count=1 isolate=true cli=claude model=claude-opus-5-5 effort=high task_id=<qa-task>`.
...
`awaiting_merge` is `cas__task action=request_changes id=<task-id>` — it reopens the
...
```
cas__factory action=sync_all_workers branch=epic/<slug>
```
...
- **Stale outbox replays:** Workers may send duplicate stale messages due to outbox replay. Before acting on a blocker notification or status change, check the task's current state with `cas__task action=show` — the message may be outdated.
...
1. Verify every child is closed and merged: `cas__factory action=epic_status id=<epic-id>` (the same source as the epic close gate; a `status=open` list misses `in_progress`, `blocked`, and `awaiting_merge`).
...
6. **Isolated mode only**: every lane already landed on the epic branch in Phase 3, before the gate ran. Once the review loop is clean and the gate exits 0, reclaim each lane's worktree (can be 10GB+ each); this end-of-lane consume is where `cleanup=true` is correct:
   ...
   cas__factory action=worktree_merge id=<worker> task_id=<task-id> cleanup=true
   ```
...
8. Shut down the epic's workers by name: `cas__factory action=shutdown_workers worker_names=<worker>[,<worker>...]`
