...
1. Search before planning — check all three sources for prior art:
   ...
   Grep pattern="<feature-name>" or cas__search action=search query="<keywords>" scope=code
   ```
...
cas__coordination action=spawn_workers count=1 isolate=true cli=claude model=claude-haiku-4-5-20251001 effort=low
...
cas__coordination action=spawn_workers count=1 isolate=true cli=claude model=claude-opus-5 effort=high
...
5. Run the lightweight per-merge gate. Do **not** run the full multi-persona
   ...
   - Record the audit trail:
     `cas__verification action=add task_id=<task-id> status=approved summary="<per-merge gate: diff read + proof checked>"`.
...
6. Bring the other worktrees onto the updated **local** branch (not `origin/`):
   ```
   cas__coordination action=sync_all_workers branch=<base-branch>
   ```
...
7. Clear completed worker's context: `cas__coordination action=clear_context target=<worker>`
8. Assign next task
...
6. **Isolated mode only**: land the lanes and reclaim the worktrees (can be 10GB+ each) only after the review loop is clean and the full gate exits 0. This is the end-of-lane consume, so `cleanup=true` is correct here:
   ...
   cas__coordination action=shutdown_workers count=0
   ```
...
8. Shutdown workers: `cas__coordination action=shutdown_workers count=0`
