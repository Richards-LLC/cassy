...
cas__coordination action=spawn_workers count=1 isolate=true cli=codex model=gpt-6-sol effort=medium task_id=<task-id>
```
...
1. Spawn workers:
   ```
   cas__coordination action=spawn_workers count=N isolate=true cli=codex model=gpt-6-sol effort=medium
   ```
...
cas__coordination action=spawn_workers count=1 isolate=true cli=codex model=gpt-6-luna effort=xhigh
...
3. Confirm they are live: `cas__coordination action=worker_status summary_mode=true`
...
`cas__coordination action=worktree_merge` is the worker merge path. It resolves
...
2. Confirm: `cas__coordination action=epic_status id=<focused-epic>` and/or `cas__task action=list status=awaiting_merge`.
   ...
   **User-facing delivery? Independent QA first (cas-619f).** When the park reports
   ...
   `cas__coordination action=spawn_workers count=1 isolate=true cli=claude model=claude-opus-5-5 effort=high task_id=<qa-task>`.
...
```
cas__coordination action=sync_all_workers branch=epic/<slug>
```
...
1. Verify every child is closed and merged: `cas__coordination action=epic_status id=<epic-id>` (the same source as the epic close gate; a `status=open` list misses `in_progress`, `blocked`, and `awaiting_merge`).
...
6. **Isolated mode only**: every lane already landed on the epic branch in Phase 3, before the gate ran. Once the review loop is clean and the gate exits 0, reclaim each lane's worktree (can be 10GB+ each); this end-of-lane consume is where `cleanup=true` is correct:
   ...
   cas__coordination action=worktree_merge id=<worker> task_id=<task-id> cleanup=true
   ```
...
8. Shut down the epic's workers by name: `cas__coordination action=shutdown_workers worker_names=<worker>[,<worker>...]`
