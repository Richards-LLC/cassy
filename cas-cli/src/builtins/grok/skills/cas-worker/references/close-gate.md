...
Before `cas__task action=close`, run these two commands in your own worktree and read the output:
...
**Crossed-message freshness handshake.** After any push, `MERGE REQUIRED`, or late amendment—and before any corrective commit—run `cas__coordination action=inbox_poll` repeatedly to `No unread messages`, re-read the task, check `git merge-base --is-ancestor <delivered-tip> <target-tip>`, and inspect whether the task is already closed/merged. If the delivery already landed or the task closed, re-close or stop; do not edit stale state.
...
- **Counter is stateless** — derive it at close time by querying `cas__task action=list assignee=<self> epic=<current-epic> status=closed` and checking whether `(count + 1) % 3 == 0` (the `+1` is for the task you're about to close).
