...
4. **Use the MCP system actions for proxy administration.** These mutate
   ...
   ```text
   cas__system action=proxy_add name=docs transport=http url=https://docs.example.test/mcp
   cas__system action=proxy_remove name=docs
   cas__system action=proxy_list
   cas__system action=proxy_health
   ```
...
5. **Verify capability, not just connectivity.** Discover the proxy surface
   with `cas__mcp_search` using `server:<name>`, count the advertised
   tools, compare them with the expected set, then make one cheap read-only
   call through `cas__mcp_execute`. A green connection with zero or one
