...
- Every user-facing subtask has a non-empty `demo_statement`; `mcp__cas__task action=create` enforces this for configured labels. Internal/unlabeled tasks and epics are unaffected, and `supervisor_override=true` is reserved for an explicit supervisor exception.
