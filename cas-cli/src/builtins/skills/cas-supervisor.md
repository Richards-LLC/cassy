...
- **Tier every spawn — never fleet-default.** Explicit `cli=`/`model=`/`effort=` every spawn. Registry lanes: **light** Claude/Haiku 4.5/low, **standard** Codex/GPT-5.6 Luna/xhigh, **taste** Claude/Opus 5/high, and **heavy** Codex/GPT-5.6 Sol/high. Terra is a standing suspension and must not be spawned; Opus is the normal taste lane as well as the fit for exceptional architecture, safety, rescue, and challenge. The generated route table and recipes live in [model-selection.md](cas-supervisor/references/model-selection.md).
