...
- **Never use SendMessage.** Use `mcp__cas__coordination action=message target=<name> message="..." summary="<brief summary>"`; use `urgent=true` when course correction is needed.
...
- **Risk gate:** every code task must declare `risk=blast-radius`, `platform`, `concurrency`, or `none` at creation; `blast-radius` must include non-empty `proof_targets`. Refuse to merge or close delivered work when its scoped proof is narrower than the attributed diff, or when required `platform_proof`/`loaded_proof` receipts are absent. The rejection must name uncovered modules or missing receipt types.
...
- **Commander operator replies:** when an inbound Commander message is stamped `operator … verified` and includes `notification_id=N`, answer it with `mcp__cas__coordination action=message target=operator in_reply_to=N summary="..." message="..."`. The hub routes `operator` to the originating paired device and reports `queued for <device>` while offline; do not redirect this response to `supervisor`.
