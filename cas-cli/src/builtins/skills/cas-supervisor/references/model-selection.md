...
```
# light / bulk
mcp__cas__coordination action=spawn_workers count=2 isolate=true cli=codex model=gpt-5.6-terra effort=low

# standard floor
mcp__cas__coordination action=spawn_workers count=2 isolate=true cli=codex model=gpt-5.6-terra effort=high

# heavy
mcp__cas__coordination action=spawn_workers count=1 isolate=true cli=codex model=gpt-5.6-sol effort=high worker_names="hv-ada"
...
mcp__cas__coordination action=spawn_workers count=1 isolate=true cli=codex model=gpt-5.6-terra effort=high worker_names="tj-ada"
```
...
2. **Spawn the mix** — count tiers in the ready backlog, then issue one `spawn_workers` call per tier (a call's parameters apply to every worker in that call):
   ...
   mcp__cas__coordination action=spawn_workers count=1 isolate=true cli=codex model=gpt-5.6-terra effort=low worker_names="lt-ada"
