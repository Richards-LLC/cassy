...
`mcp__cas__factory action=gc_cleanup force=true older_than_secs=86400` terminally
...
- Check liveness with `mcp__cas__factory action=worker_status` before declaring a worker idle or dead.
...
Run `mcp__cas__factory action=worker_status`, then the `is-wedged` / `debug` / `kill` triad above. Salvage committed work with `mcp__cas__factory action=worktree_merge id=<worker> task_id=<task-id>` before any cleanup, and message the replacement worker what already landed.
...
**Diagnosis:**
1. Confirm a fresh heartbeat with `mcp__cas__factory action=worker_status`
...
shutdown requests. Run `mcp__cas__factory action=restart_spawn_queue`. It
...
`mcp__cas__factory action=worker_status` prints a `context:` line per worker, banded by the share of the model's context window in use:
...
2. Wait for the commit confirmation (watch `mcp__cas__factory action=worker_activity`).
...
4. Once work is committed and the worker is idle: `mcp__cas__factory action=recycle_worker target=<worker>`. It restarts the same name with its recorded recipe and keeps the worktree.
...
2. Shut down the affected worker immediately: `mcp__cas__factory action=shutdown_workers worker_names=<worker>`. Do not attempt to salvage the session.
