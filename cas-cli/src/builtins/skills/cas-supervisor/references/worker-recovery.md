...
2. If the task must be intentionally ended after delivery and evidence are confirmed, use `mcp__cas__task action=cancel id=<task-id> reason="..."`; use `request_changes` for rework, or `reset` only for an orphaned dead session.
