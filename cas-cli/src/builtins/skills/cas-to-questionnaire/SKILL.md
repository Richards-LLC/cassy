...
Propose `docs/questionnaires/YYYY-MM-DD-<recipient>-<topic>.md` (or the location the user names) and write the questionnaire there once the user confirms. State its purpose, sender, recipient, and how answers will be used; give only enough context to orient the recipient; order one-idea questions by importance; provide an answer stub and a short “why this matters” only where useful; finish with a catch-all. Preserve task or decision context through `task`, `spec`, or `memory`, not a parallel tracker or context-file convention.
