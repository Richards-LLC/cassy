...
5. **Keep the invariant constraints.** One file, no network, semantic HTML, contrast ≥ 4.5:1
   ...
   any other surface. Done when a JS-disabled reload and a print preview lose nothing.
6. **Critique with the rubric before merge.** Score the artifact 1–5 on each dimension in
   ...
   the session will see) merges only with distinctiveness, fit, and hierarchy each ≥ 4; below the
   floor, revise the brief's *distinctive move* and re-render — do not ship and annotate. Done when
   the table is in the brief and every floor holds.
