...
**File size limit:** `mecha_post kind: "file"` accepts files up to **1 MiB (1,048,576 bytes)** of raw file content. Anything larger fails at the hub with HTTP 413 or `file_too_large`. Check the file size before posting and shrink or split an oversized file; `scripts/release-report-post.py` refuses one up front, naming the limit and the file size.
