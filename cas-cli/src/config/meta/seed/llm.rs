...
pub(super) fn register_llm(registry: &mut ConfigRegistry) {
    ...
    registry.register(ConfigMeta {
        ...
        section: "llm",
        name: "Worker Model",
        description: "Override model for worker agents. Fallback chain: [llm.worker.model] → [llm.model] → stock worker default (gpt-6-sol at effort=medium). Terra is suspended pending operator re-enable.",
        value_type: ConfigType::String,
        default: "(inherit)",
        constraint: Constraint::None,
        advanced: false,
