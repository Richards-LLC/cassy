...
// create_cas_skill is available but not re-exported - used internally in tests
...
use crate::types::{Rule, RuleStatus, Scope};
...
impl Syncer {
    ...
    pub fn sync_all(&self, rules: &[Rule]) -> Result<SyncReport, CasError> {
    ...
    pub fn remove_rule(&self, rule_id: &str) -> Result<(), CasError> {
    ...
    pub fn sync_rule_with_scope(&self, rule: &Rule) -> Result<bool, CasError> {
    ...
    fn sync_rules_to_dir(&self, target_dir: &Path, rules: &[Rule]) -> Result<SyncReport, CasError> {
        let mut report = SyncReport::default();
    ...
    pub fn list_all_synced(&self) -> Result<(Vec<String>, Vec<String>), CasError> {
    ...
    fn list_synced_from_dir(&self, dir: &Path) -> Result<Vec<String>, CasError> {
        let mut ids = Vec::new();
