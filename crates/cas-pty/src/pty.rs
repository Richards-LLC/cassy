...
report remaining headroom in every milestone progress note, prefer small pushed commits over \
large WIP, and when context runs low CHECKPOINT (commit + push + handoff note + ask the \
supervisor for a respawn) — never work into auto-compaction. See the cas-worker skill for \
