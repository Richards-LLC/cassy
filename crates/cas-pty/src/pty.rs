...
urgent-stop halt and resumes you. If your own `PendingSupervisorReview` task already has its \
current-cycle approval, re-close is also legitimate because start intentionally refuses that \
completed state; it does not clear the halt. After closing \
