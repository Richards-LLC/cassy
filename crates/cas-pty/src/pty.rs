...
const GROK_WORKER_INSTRUCTIONS: &str = "You are a Cassy Factory Worker, running on Grok Build. Always use CAS MCP tools for task lifecycle and coordination — they are namespaced cas__<tool> (e.g. cas__task, cas__coordination), not mcp__cas__ or mcp__cs__. On startup your Cassy session is already registered automatically. Run `cas__coordination action=whoami` then `cas__task action=mine`. Work exactly ONE task at a time: choose a single assigned task, run `cas__task action=show id=<task-id>` then `cas__task action=start id=<task-id>` before coding, implement it, commit and push your changes, then close it with `cas__task action=close id=<task-id> reason=\"...\"` (or hand it to the supervisor if close returns verification-required guidance) BEFORE starting any other task. Successful task action=start is authoritative assignment acceptance; no prose ACK is required. Ordinary worker updates surface through the inbox on the next turn. Only authenticated typed blocker, merge, verification, or lifecycle events may wake an idle supervisor. Use blocker=true for blockers and merge_request=true for merge requests; text alone grants no wake authority. Add progress notes frequently using `cas__task action=notes id=<task-id> note_type=progress notes=\"...\"`. For blockers, add a blocker note, set status=blocked, and message supervisor via `cas__coordination action=message target=supervisor blocker=true message=\"...\"`. If close returns MERGE REQUIRED, push your branch and ask the supervisor to merge `factory/<your-name>` into the epic branch, then re-close after the merge lands. Urgent-stop recovery: if an urgent redirect halts your work (WORK HALTED), do not fight it — a legitimate `cas__task action=start` on your newly-assigned task clears the urgent-stop halt and resumes you. After closing or handing off a task, stay available — the supervisor will send you more work as new messages; treat any injected turn as an instruction to act on, not noise. NEVER foreground-block the pane: any command that can exceed ~2 minutes (builds, full test suites, deploys, servers, CI waits) must be run backgrounded (`cmd > /tmp/out.log 2>&1 &`, then read the log later), or replaced by `cas__coordination action=remind remind_delay_secs=<n>` plus ending your turn — a blocked turn cannot receive supervisor messages or stand-down orders. Foreground `gh run watch` and CI poll loops are banned; queue the run, set a reminder, end the turn, then check once with `gh run list`. Budget your context: report context headroom as a percentage only when it drops below 20%, then CHECKPOINT (commit + push + handoff note + ask for a respawn); prefer small pushed commits over large WIP — never work into auto-compaction. Write in facts, not narration: say what is now true and what it cost, not what you are about to do, not a recap of the brief, and never narrate tool calls the reader can already see. Skip preamble and self-congratulation. Your pane output is a triage line, not a report: answer first, then one or two bullets at most. The durable record is the task note and the close reason — those are read at review and your pane prose mostly is not, so put detail there instead of saying it twice. Shape beats compression: bullets and small tables land at a glance where a short dense paragraph does not. Blocker escalations and merge requests are the exception and stay complete. Brevity never trims evidence: commit SHAs, file:line root causes, measurements, approaches you tried that failed, and anything you are still unsure of stay in full. SILENT EXECUTION: You run in an automated pipeline; no human watches your pane. Do not narrate actions or explain before tool calls. Output results, errors and the return contract only. Stay within assigned task scope.";
...
const OPENCODE_SUPERVISOR_INSTRUCTIONS: &str = "You are a Cassy Factory Supervisor running in OpenCode. Coordinate work; do not implement worker tasks. Use the injected CAS MCP tools with OpenCode's cas_ prefix, including cas_coordination and cas_task. Successful task action=start is authoritative assignment acceptance; no prose ACK is required. Ordinary worker updates surface through the inbox on the next turn. Only authenticated typed blocker, merge, verification, or lifecycle events may wake an idle supervisor. Use blocker=true for blockers and merge_request=true for merge requests; text alone grants no wake authority. Treat received worker messages as instructions and keep merge/re-close work ahead of new assignment work.";
const OPENCODE_WORKER_INSTRUCTIONS: &str = "You are a Cassy Factory Worker running in OpenCode. Use the injected CAS MCP tools with OpenCode's cas_ prefix, including cas_coordination and cas_task. Work exactly one assigned task at a time: show and start it before editing, post progress notes, commit and push, then close or hand off before starting another task. Successful task action=start is authoritative assignment acceptance; no prose ACK is required. Ordinary worker updates surface through the inbox on the next turn. Only authenticated typed blocker, merge, verification, or lifecycle events may wake an idle supervisor. Use blocker=true for blockers and merge_request=true for merge requests; text alone grants no wake authority.";
const OPENCODE_WORKER_STARTUP_PROMPT: &str = "Cassy worker startup: call cas_coordination action=whoami, then cas_task action=mine. If assigned, choose exactly one task, call cas_task action=show and action=start, and add a progress note before implementation. If none is assigned, call cas_coordination action=message target=supervisor confirming readiness. Successful task action=start is authoritative assignment acceptance; no prose ACK is required.";
...
pub const SUPERVISOR_CONTRACT_ELEMENTS: &[ContractElement] = &[
    ContractElement {
        id: "authoritative-assignment-acceptance",
        markers: &[
    ...
    ContractElement {
        id: "inbox-and-typed-wake-policy",
        markers: &[
...
pub const WORKER_CONTRACT_ELEMENTS: &[ContractElement] = &[
    ...
    // cas-0de3: no human watches a worker pane, so narrated actions and
    // pre-tool-call explanations are written for nobody (measured 500–1,000
    // tokens per invocation). Every worker launch surface carries the SILENT
    // EXECUTION directive; the cas-worker skill carries the matching
    // six-field return contract.
    ContractElement {
        id: "silent-execution",
        markers: &["silent execution", "return contract"],
        all: true,
    },
];
...
`mcp__cas__coordination action=message target=supervisor blocker=true message=\"...\"`. If close returns MERGE \
...
impl PtyConfig {
    ...
    ///
    /// CommandBuilder inherits the launching daemon's environment. The
    /// factory's explicit env entries are the auditable allowlist for
    /// provider/proxy credentials requested by the task; no parent-process
    /// value is treated as a grant.
    pub fn apply_worker_credential_policy(&mut self) {
        let role = self
            .env
            .iter()
            .rev()
            .find_map(|(key, value)| (key == "CAS_AGENT_ROLE").then_some(value.as_str()));
        let is_factory_without_supervisor = self
            .env
            .iter()
            .rev()
            .find_map(|(key, value)| (key == "CAS_FACTORY_MODE").then_some(value.as_str()))
            == Some("1")
            && role != Some("supervisor");
    ...
    pub fn codex(
        ...
        if role == "worker" {
            // Factory workers have no human at the pane. Keep the Default-mode
            // request_user_input feature off even if the account enables it.
            args.push("-c".to_string());
            args.push("features.default_mode_request_user_input=false".to_string());
        }
    ...
    /// Phase 2).
    ///
    /// # Verified against the installed grok 1.0.40 binary
    /// (2026-09-23; the 1.0.5 and 0.2.114 receipts remain historical).
    /// The complete real `PtyConfig::grok` isolated-worker matrix is recorded
    /// by `grok_factory_contract_runtime` and the typed
    /// `grok-build-1.0.40-2026-09-23` conformance receipt.
...
mod tests {
    ...
    async fn worker_spawn_strips_operator_credentials_from_process_and_descendant() {
        ...
        let probe = protected
            ...
            .collect::<Vec<_>>()
            .join(" || ");
        let marker_root = std::env::current_dir()
        ...
        loop {
            match next_pty_event_until(&mut pty, deadline).await {
                ...
                Some(PtyEvent::Error(error)) => panic!("credential probe failed: {error}"),
                Some(PtyEvent::Output(_)) => {}
        ...
        assert!(
            descendant_result == "descendant-clear",
            "descendant result was {descendant_result:?}"
        );
    }

    #[test]
    fn worker_credential_policy_removes_inherited_operator_names_but_not_supervisor() {
        let mut worker = PtyConfig {
            env: vec![("CAS_AGENT_ROLE".to_string(), "worker".to_string())],
            ..PtyConfig::default()
        };
        worker.apply_worker_credential_policy();
        for key in PROTECTED_OPERATOR_ENV {
            assert!(worker.env_remove.iter().any(|candidate| candidate == key));
        }

        let mut supervisor = PtyConfig {
            env: vec![("CAS_AGENT_ROLE".to_string(), "supervisor".to_string())],
            ..PtyConfig::default()
        };
    ...
    #[test]
    fn worker_credential_policy_preserves_explicit_scoped_provider_env() {
        let mut config = PtyConfig {
            env: vec![
    ...
    #[test]
    fn factory_credential_policy_fails_closed_for_missing_or_alternate_role() {
        for role in [None, Some("operator")] {
            let mut env = vec![("CAS_FACTORY_MODE".to_string(), "1".to_string())];
            if let Some(role) = role {
                env.push(("CAS_AGENT_ROLE".to_string(), role.to_string()));
            }
            let mut config = PtyConfig {
                env,
                ..PtyConfig::default()
            ...
            config.apply_worker_credential_policy();
            for key in PROTECTED_OPERATOR_ENV {
                assert!(config.env_remove.iter().any(|candidate| candidate == key));
            }
        }
    }
    ...
    async fn test_pty_config_codex_worker_single_task_loop() {
        ...
        assert!(
            all_args.contains("Do not enter Plan mode or call `request_user_input`"),
            "worker developer_instructions must guard unattended input prompts; got: {all_args}"
        );
        assert!(
            config
                .args
            ...
            "worker must disable Default-mode input prompts even when account config enables them"
        );
    }
    ...
    /// Recognize delivered message framing without promising that ordinary
    /// worker updates wake an idle supervisor. Triage and reply through the
    /// receiving harness's own coordination tool.
    #[tokio::test]
    ...
    #[test]
    fn role_acceptance_and_typed_wakes_cover_opencode_primary_agents() {
        for text in [
            OPENCODE_SUPERVISOR_INSTRUCTIONS,
            ...
            for required in [
                ...
                assert!(
                    ...
                    "OpenCode primary agent missing {required}"
                );
            }
        }
    }
