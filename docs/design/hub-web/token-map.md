...
| 3 | `--bg-raised` #1B202B | **keep**, derived: `color-mix(in srgb, surface 96%, ink)` | ≈ #F5F5F5 | ≈ #21242C | the house has two surface steps; the console needs a third for rows, inputs and buttons on `surface`; deriving it from `surface`+`ink` keeps it one scheme-aware expression, not a literal — **since design polish P15:** derived from `bg` 92% + `ink` (≈ #E5E3DE / #23252A) so chips stay warm on paper |
| 4 | `--bg-terminal` #0C0E13 | **keep**, same value in both schemes | #0C0E13 | #0C0E13 | terminal wells, transcript, code wells and the connection log stay dark under a light UI; Ghostty's ANSI palette is unchanged and would not survive a light well (brief: *Not changing*) — **as shipped (Unit 7):** the dark wells are `.terminal-mount`, `.transcript`, `.terminal-search input`, `.attention-payload pre` and `.connection-log pre` (styles.css:840/852/937/1497/1806); dialog inputs left the well list in design polish P4 and are `--panel` fields with a `--line-strong` edge; the command-palette input is `--bg-panel` (`.command-palette input`, :1858, wins on specificity) and `.pair-code` is transparent on the dialog surface — neither is a well |
| 5 | `--bg-hover` #222836 | **keep**, derived: `color-mix(in srgb, surface 92%, ink)` | ≈ #EBEBEB | ≈ #2A2D36 | hover step above `--bg-raised`; same derivation, one step further — **since P15:** `bg` 88% + `ink` (≈ #DDDAD6 / #2C2D32) |
| 6 | `--bg-active` #2A3142 | `color.*.verdict-soft` | #DDE1F7 | rgba(169,179,255,.16) | selection, the active tab and the active machine tile are "the highlighted row" (since design polish P3 the primary button is `--you-bg`, not this band) — the band the house puts behind the decisive interval |
