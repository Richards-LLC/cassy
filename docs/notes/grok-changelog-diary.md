...
- **Gap:** the validated pin remains **1.0.5** because the complete live
  ...
  **cas-ef93**, including the urgent-interrupt path, and must not be inferred
  from this diary update. Grok 1.0.24 stopped Esc from cancelling a turn; Cassy
  is already insulated because `Pane::break_turn`
  (`crates/cas-mux/src/pane/mod.rs:1462`) writes the harness's cancel bytes and
  the Grok backend sends Ctrl+C, not Esc
  (`crates/cas-mux/src/backend/grok.rs:70-72`, since cas-7f6f). The matrix
  still owes the live confirmation that an urgent worker redirect breaks a
  Grok 1.0.40 turn. The earlier 0.2.102–0.2.103 and 0.2.107–0.2.111 gaps
...
- **Esc no longer cancels a running turn and instead reminds users to use
  ...
  against 1.0.40 remains part of **cas-ef93** before the validated pin
  advances.
