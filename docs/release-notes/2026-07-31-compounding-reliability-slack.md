...
- Was: delivery merges compared the target tip at preflight only. Now: a canonical repo+ref advisory lock plus compare-and-swap holds through the merge, first-parent topology is verified against the reviewed SHA, and drift rolls back via `update-ref` Cassy with a typed recoverable state.
