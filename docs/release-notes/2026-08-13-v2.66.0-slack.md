...
**Top-level:**
Live on production — **User** — Cassy v2.66.0 is out: adding machines and handing work across projects now happen through guided, trackable flows instead of loose links and manual follow-up.
...
- Was: work owned by another project had to be copied or handed over manually, so its destination and outcome were easy to lose. Now: Cassy sends one proposal to the intended project, shows whether it was accepted or declined, and keeps dependent work honestly blocked until the external task is finished.
