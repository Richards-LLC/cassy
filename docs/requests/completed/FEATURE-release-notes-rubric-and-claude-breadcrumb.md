...
> **Disposition (2026-08-07, cas-ab75):** DELIVERED — filed as [#65](https://github.com/pippenz/cas/issues/65) (closed completed). Verified on `main`: `dd4487aa` — `cas-cli/src/builtins/skills/release-notes/` ships, and the Cassy-managed `docs/release-notes/RUBRIC.md` breadcrumb is injected from `cas-cli/src/builtins.rs` + `cas-cli/src/cli/init/docs_and_skill.rs`. Archived.

> Migrated to GitHub Issues: [#65](https://github.com/pippenz/cas/issues/65)

# Feature Request: ship a release-notes / Slack-announcement rubric + a Cassy-managed CLAUDE.md breadcrumb to it

## What we need

Cassy should **own a release-notes / Slack-announcement rubric** as a framework-level artifact (the announcement analog of what `codemap`/`project-overview`/the proposed `design-spec` are for their domains), and **breadcrumb to it from the Cassy-managed `CLAUDE.md` block** so every Cassy project picks it up automatically — including cas-src's own checked-in `CLAUDE.md`.
...
**Every PR merged to `staging` or `main` must be announced in Slack following the rubric.** Today that's a per-project convention only the Gabber `CLAUDE.md` states; it should be a Cassy-managed expectation that travels with every project.
...
1. **Ship the rubric** as a framework artifact Cassy can drop into a project (like the `CAS:BEGIN/END` block) — a canonical `docs/release-notes/RUBRIC.md` template.
2. **Breadcrumb in the Cassy-managed `CLAUDE.md` block:** add a short "Release notes: on every staging/main merge, announce in Slack per `docs/release-notes/RUBRIC.md`" line + link inside the `<!-- CAS:BEGIN ... CAS:END -->` section Cassy already manages, so it propagates to all projects.
...
- The "announce on every staging/main merge" expectation is easy to forget unless it's in the Cassy-managed `CLAUDE.md` block.
