...
test.describe("Conversation composer", () => {
  test("Composer preserves and edits the seeded draft", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "patient-pelican-9 cas-src" })).toBeVisible();
    ...
    const attachment = page.getByRole("button", { name: "Attach a file (not yet supported)" });
