...
test("HUB-J4 read the conversation history", async ({ page, journey }) => {
  const hub = await journey.hub({
    ...
    history: {
      [PELICAN]: [
        { has_earlier: true, next_before: 20, messages: [you(21, "Is the release ready to cut?", 2)], replies: [sup(22, 21, "Yes. The gate is green on the release branch.", 1.9, [file("art-report", "Release report card.pdf"), file("art-local-draft", "Draft notes.pdf")])] },
