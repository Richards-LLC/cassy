...
export async function createGhosttyTerminalSurface(
  ...
  const surface = await GhosttyTerminalSurface.create(mount, {
    ...
    ...(callbacks.onRender ? { onRender: callbacks.onRender } : {}),
  ...
  return {
    ...
    transcript: {
      rows: () => surface.snapshotRows(),
      theme: () => ({ foreground: theme.foreground, background: theme.background }),
      hasScrollbackAbove: () => surface.hasScrollbackAbove(),
      scrollRows: (delta) => surface.scrollRows(delta),
      scrollToBottom: () => surface.scrollToBottom(),
      focus: () => surface.focus(),
    },
