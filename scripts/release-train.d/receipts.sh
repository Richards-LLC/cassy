...
release_train_receipts() {
    ...
    local repo_slug pr_url pr_number pr_id queue_output commit_sha
    ...
    if [[ ! -d "$receipt_worktree/.git" ]]; then
    ...
    queue_output="$( "${CAS_RELEASE_TRAIN_GH:-gh}" api graphql \
    ...
    if ! printf '%s' "$queue_output" | grep -q '"state"'; then
        printf 'ERROR receipts queue: PR #%s returned no merge-queue state\n' "$pr_number" >&2
        return 1
    fi
